...
expName = 'Motor Sequencing Experiment'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\anany\\Desktop\\LAB\\Motor Sequencing\\Motor Sequencing Experiment_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "random__" ---
    fixation_1 = visual.ShapeStim(
        win=win, name='fixation_1', vertices='cross',units='pix', 
        size=(10, 10),
        ori=0.0, pos=(0, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=0.0, interpolate=True)
    ...
    line_04 = visual.Rect(
        ...
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-4.0, interpolate=True)
    probe_01 = visual.ShapeStim(
        win=win, name='probe_01',units='pix', 
        size=(10, 10), vertices='triangle',
        ori=0.0, pos=[0,0], anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-5.0, interpolate=True)
    ...
    trials = data.TrialHandler(nReps=50.0, method='sequential', 
        ...
        trialList=data.importConditions('Sequence Conditions - Sheet1.csv'),
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "random__" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('random__.started', globalClock.getTime(format='float'))
        probe_01.setPos((pos, 60))
        # create starting attributes for key_resp_2
        key_resp_2.keys = []
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        # keep track of which components have finished
        random__Components = [fixation_1, line_01, line_02, line_03, line_04, probe_01, key_resp_2]
        for thisComponent in random__Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # if probe_01 is starting this frame...
            if probe_01.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
                # keep track of start time/frame for later
                probe_01.frameNStart = frameN  # exact frame index
                probe_01.tStart = t  # local t and not account for scr refresh
                probe_01.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(probe_01, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'probe_01.started')
                # update status
                probe_01.status = STARTED
                probe_01.setAutoDraw(True)
            
            # if probe_01 is active this frame...
            if probe_01.status == STARTED:
                # update params
                pass
        ...
        # --- Ending Routine "random__" ---
        for thisComponent in random__Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('random__.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "random__" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
